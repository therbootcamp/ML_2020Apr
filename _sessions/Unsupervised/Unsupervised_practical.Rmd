...
4. Ganz unten im Output steht eine Liste der im Objekt enthaltenen Elemente. Verwende `gap2007_km$XX` um das Element `cluster` als eigenes Objekt `clusers` und das Element `centers` als eigenes Objekt `centers` zu speichern. 
...
5. Verwende den Code unten eine Abbildung mit der Daten mit Clusterzuweisungen zu erstellen. Wenn ihr damit vertraut seit, dürft ihr natürlich auch das wesentlich `ggplot2` benutzen.
...
4. Auf Basis des Verlaufs erscheinen verschiedene werte für *k* plausibel: 1, 3, oder 7. Verwendet die `cDistance()` Funktion aus dem `cstab` Paket im Code unten um Schätzungen für *k* innerhalb der Werte `2:20` auf Basis der Gap- und Slope-Statistik zu erhalten. 
...
1. Zum Abschluss, verwende `Mclust()` aus dem `mclust` Paket um über Gaussian Mixture Modell die Cluster zu bestimmen. Arbeite hier mit dem nicht-standardisierten Datensatz `gap2007`. Dies  ist möglich, weil Gaussian Mixtures die Skalen der Variablen automatisch berücksichtigt. 
...
3. Der Output verrät relativ wenig, nur welche Elemente enthalten sind. Verwendet `table(gap2007_gm$classification)` um einen Überblick über die Clusterzuweisungen zu erhalten. Wie viele Cluster wurden identifiziert?    

4. Verwendet das `classification` Element um wie üblich die Daten mit den Clusterzuweisungen zu plotten. 
...
5. Führt nun alternativ `plot(gap2007_gm, what = 'classification')` aus, um den eigenen Plot des `mclust` Pakets zu sehen. 
...
6. Versucht nachzuvollziehen was der `mclust` Plot euch zeigt. Erinnere dich, die Ellupsen sind Normalverteilungen, die jeweils eigene Skalen und Feature-Zusammenhänge berücksichtigen können.  

7. Eine interessante Eigenschaft von Gaussian Mixtures ist, dass man direkt die Unsicherheit der Clusterzuweisung evaluieren kann. Führt `plot(gap2007_gm, what = 'uncertainty')` aus. Die Grösse der Punkte zeigen an, wie gross die Unsicherheit (oder Rivalität) in der Zuweisung der Punkte zu den Clustern war. 
### X - Challenges: Modellselektion Gaussian mixtures

1. : Eine nützliche Eigenschaft der `Mclust()` Funktion ist, dass parallel verschieden komplexe Varianten des Modells mit verschiedenem *k* geschätzt werden und dass am Ende nicht nur das beste *k* ausgewählt wird, sondern die beste Kombination von *k* und Modell. Ihr erhalten einen Überblick über den Prozess mit `plot(gap2007_gm, what = 'BIC')`.
...
Der `credit` Datensatz ist ein Ausschnitt des Öffentlich verfügbaren [*Credit Card Dataset*](https://www.kaggle.com/arjunbhasin2013/ccdata). Der Datensatz beinhaltet die 8 Feature, die einen Auschnitt des Verhaltens von 8636 Kreditkartenkunden beschreiben.  
