...
raw <- ggplot(dat_great, aes(x = x0, y = y0)) + geom_point(col = baselrbootcamp_cols("grey"), size = 2) +
  ...
  ylim(c(0, 5)) +
  labs(title = "Rohdaten",
       x = "Feature", y = "Abhängige Variable")
...
logreg1 = ggplot(dat_great, aes(x = x0, y = y0)) + geom_point(col = baselrbootcamp_cols("grey"), size = 2) +
  theme_minimal() +
  labs(x = "Feature", y = "Abhängige Variable")
...
<ul>
<li class="m1"><span>Modelle können wir uns als <high>Familie von Modellen</high> vorstellen, wobei jede Parameterkombination ein unterschiedliches Modell definiert</span></li>
<li class="m2"><span>Wenn wir ein Modell fitten, <high>identifizieren wir</high> von der Familie von Modellen <high>dasjenige Modell, welches unsere Daten am besten abbildet</high>.</span></li>
</ul>
...
<ul>
<li class="m1"><span>Eines der <high> wichtigsten Konzepte</high> in der Statistik und im maschinellen Lernen.</span></li>
<li class="m2"><span>Die Verlustfunktion definiert eine <high>Zusammenfassung der durch ein Modell begangenen Fehler</high>.</span></li>
</ul>
...
<ul style="margin-bottom:-20px">
  ...
  <br><br>
  <ul class="level">
    <li><span><high>Mittlere Quadratsumme der Abweichungen</high> zwischen vorhergesagten und wahren Werten.</span></li>
  </ul>
  </span></li>
</ul>
...
<ul>
  ...
  <br><br>
  <ul class="level">
    <li><span><high>Mittlere absolute Abweichungen</high> zwischen vorhergesagten und wahren Werten.</span></li>
  </ul>
  </span></li>
</ul>
...
<ul style="margin-bottom:-20px">
  ...
  <br><br>
  <ul class="level">
    <li><span>Regressionsprobleme beinhalten die <high>Vorhersage eines quantitativen Features</high>.</span></li>
    <li><span>Z.B., die Vorhersage des Cholesterinlevels als Funktion von Alter.</span></li>
  </ul>
  </span></li><br>
  <li class="m2"><span><b>Klassifikation</b>
  <br><br>
  <ul class="level">
    <li><span>Klassifikationsprobleme beinhalten die <high>Vorhersage eines kategorialen Features</high>.</span></li>
    <li><span>Z.B., die Vorhersage der Ursache von Brustschmerzen als Funktion von Alter.</span></li>
  </ul>
  </span></li>
</ul>
...
<ul style="margin-bottom:-20px">
  <li class="m1"><span><b>Distanz</b>
  <br><br>
  <ul class="level">
    <li><span>LogLoss wird – wie MSE und MAE –  als Distanzmass <high>zum Fitten von Parametern</high>, verwendet. Es ist somit eine Alternative zu MSE und MAE.</span></li>
  </ul>
  </span></li>
</ul>
...
<ul>
  <li class="m2"><span><b>Überlappung</b>
  <br><br>
  <ul class="level">
    <li><span>Die 0-1 Verlustfunktion tested, ob die <high>vorhergesagte Kategorie, der tatsächlichen Kategorie entspricht</high>. Wird häufig wegen der <high>einfachen Interpretation</high> verwendet.</span></li>
  </ul>
  </span></li>
</ul>
...
<ul style="margin-bottom:-20px">
  <li class="m1"><span>Die Wahrheitsmatrix (<i>confusion matrix</i>) <high>enthält die Anzahl einer bestimmten Klasse zugeordneten Werte</high> als Funktion der tasächlichen Klasse.</span></li>
...
<ul>
  ...
  <li class="m2"><span>Für den Moment, verwende der Einfachheit halber <mono>method = "none"</mono>. Mehr hierzu in der <b>Optimisierung</b> Session.</span></li>
</ul>
...
<ul>
  <li class="m1"><span><mono>train()</mono> ist <mono>caret</mono>'s <high>Arbeitstier</high>, wenn es um Fitting geht. Damit können <high>über 200 Modelle</high> gefittet werden, wofür man jeweils nur das <high>method</high> Argument anpassen muss.</span></li>
</ul>
...
<ul style="margin-bottom:-20px">
  <li class="m1"><span>Die abhängige Variable muss die richtige Klasse haben:
  <br><br>
  <ul class="level">
    <li><span><high><mono>numeric</mono></high> AV &rarr; <high>Regression</high><br></span></li>
    <li><span><high><mono>factor</mono></high> AV &rarr; <high>Klassifkation</high><br></span></li>
  </ul>
  </span></li>
</ul>
...
```{r, echo = FALSE}
Loans <- tibble::tibble(Kreditausfall = c(0, 1, 0, 1, 1),
...
<ul>
  ...
  <li class="m2"><span>Greiffe auf das Modell mit <mono>mod$finalModel<mono> zu und <high>exploriere</high> das Objekt mit generischen Funktionen:</span></li>
</ul>
...
eink_mod <- train(form = einkommen ~ alter + groesse, # Formula
...
```
 ...
 <p align = "center">
<img src="image/summary.png" height=225px>
</p>
...
<ul>
  <li class="m1"><span>Die <mono>predict()</mono> Funktion <high>macht Vorhersagen</high> von einem Modell. Dazu muss man lediglich den Modelloutput als erstes Argument spezifizieren.</span></li>
</ul>
...
<ul>
  <li class="m1"><span>Die <mono>postResample()</mono> Funktion <high>erstellt eine einfache Zusammenfassung</high> der Modellperformanz bei <high>Regressionsproblemen</high>. Zu spezifizierende Argumente sind die vorhergesagten Werte und die wahren Werte.</span></li>
</ul>
...
<ul>
  <li class="m1"><span>Die <mono>confusionMatrix()</mono> Funktion erstellt eine Zusammenfassung der Modellperformanz bei <high>Klassifikationsproblemen</high>. Zu spezifizierende Argumente sind wiederum die vorhergesagten Werte und die wahren Werte.</span></li>
</ul>
