...
var countDownDate = new Date("Apr 4, 2020 17:00:00").getTime();
...
3. Um ein `train`-Objekte einzureichen, speichert es zuerst als `.RDS`-Datei, die ihr benennt mit `PSEUDONYM_VERSION.RDS`, wobei ihr `PSEUDONYM` mit einem selbstgewählten Pseudonym ersetzt und `VERSION` mit einem selbstgewählten Beschreibung des verwendeten Ansatzes. Siehe den Code unten. 
...
5. Reiche deine `.RDS` Datei über den folgenden Link ein. Wenn der Upload nicht funktioniert, dann schickt uns das Objekt oder die Objekte per [**Mail**](mailto:therbootcamp@gmail.com). 

Submit your `.RDS` file (containing your training object) via the following link:
...
6. Du darfst alle Waffen in `caret`s Arsenal verwenden. Versuche neue Modelle oder Tuning Parameter oder neue Präprozessierungs Methoden. Verwende alle oder nur einige Feature. 

7. <font style="font-color:#EA4B68">Was du nicht darfst, ist neue Variablen zu erstellen, ansonsten können wir nämlich später nicht dein Modell mit anderen Vergleichen. D.h. alle Manipulation der Feature (jenseits deren Eliminiation z.B. dadurch dass nur bestimmte Feature in der Formula für die Vorhersage ausgewählt werden) müssen über das `preProcess`-Argument der `train()` Funktion durchgeführt werden. </font>
...
|File  |Rows | Columns |
|:----|:-----|:------|
|[tweets.csv](https://raw.githubusercontent.com/therbootcamp/appliedML_2019Jan/master/1_Data/tweets_train.csv)| 2500 | 23|
