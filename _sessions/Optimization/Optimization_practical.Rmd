...
author: "<table style='table-layout:fixed;width:100%;border:0;padding:0;margin:0'><col width='10%'><col width='10%'>
  <tr style='border:none'>
    <td style='display:block;width:100%;text-align:left;vertical-align:bottom;padding:0;margin:0;border:none' nowrap>
      <font style='font-style:normal'>Machine Learning mit R</font><br>
...
3. Wie sich eine simple Regression von Lasso und Ridge unterscheiden.
...
1. Fitte eine normale Regression auf bekanntem Wege (wie z.b. im Practical zu [Prediction](_sessions/Prediction/Prediction_practical.html)), die das `Gehalt` des Baseballspielers durch alle Feature im Datensatz vorhersagt. Einziger Unterschied: Setze das `trControl` Argument auf das eben definierte `ctrl_cv` Objekt. 
...
1. Bevor du ein Regreessionsmodel mit Ridge fitten kannst, benötigst du das Kandidatensetz für die Tuning Parameter. Erstelle hierzu mit dem Code einen Vector von möglichen `lambda` Werten.  
...
3. Plotte das Objekt `gehalt_ridge` mit `plot()`. Was verrät dir der Plot? Welcher `lambda` Wert könnten optimal sein?
...
1. Fitte eine Lasso Regression, die das `Gehalt` des Baseballspielers durch alle Feature im Datensatz vorhersagt. Beginne mit dem Code für die Ridge Regression und definiere als einzige Änderung `alpha = 1`.
...
2. Plotte das Objekt `gehalt_lasso` mit `plot()`. Was verrät dir der Plot? Welcher `lambda` Wert könnten optimal sein?
...
```{r, echo = TRUE}
# Print besten lambda Parameter
gehalt_lasso$bestTune$lambda
```

4. Lasse dir mit dem Code unten die Parameter des finale Modells anzeigen. Wie beurteilst du den Output? Hat sich was im Vergleich zu Ridge verändert?
...
1. Bevor du einen Decision Tree per Cross-Validation fittest, benötigst du das Kandidatensetz für die Tuning Parameter. Erstelle hierzu mit dem Code einen Vector von möglichen `cp` Werten. 
...
2. Fitte einen Decision Tree, die das `Gehalt` des Baseballspielers durch alle Feature im Datensatz vorhersagt. Beginne mit dem bekannten Code und zusätzlich...
...
3. Plotte das Objekt `gehalt_rpart` mit `plot()`. Was verrät dir der Plot? Welcher `cp` Wert könnten optimal sein?
...
1. Bevor du einen Random Forest per Cross-Validation fittest, benötigst du das Kandidatensetz für die Tuning Parameter. Erstelle hierzu mit dem Code einen Vector von möglichen `mtry` Werten. 
...
2. Fitte einen Random Forest, die das `Gehalt` des Baseballspielers durch alle Feature im Datensatz vorhersagt. Beginne mit dem bekannten Code und zusätzlich...
...
4. Verwende `postResample()` um die Vorhersagen mit den tatsächlichen Kriteriumswerten zu vergleeicheen. Welches Modll hat in der Vorhersage der Testdaten die beste Performanz?
...
Vergleiche auf Basis der Schritte in diesem Practical die Performanz von Regression, Lasso, Ridge, Decision Tree und Random Forest für die `graduation_train` und `graduation_test` Datensätze aus dem letzten Practical. Lass die Modelle das Feature `Privatuniversität` auf Basis aller anderen Feature vorhersagen. Nicht vergessen für Klassifikation müssen das Kriterium ein `factor` sein.

1. Welches Modell ist das beste? 
2. Wie viel besser werden die Modelle durch Model Tuning? Vergleiche mit den Ergebnisse des letzten Practicals. 
3. Setzt Lasso die Gewichte einzelne Feature auf 0? Was könnte das für die Relevanz des Features bedeuten?

### Y - Challenges: Hauspreise

Vergleiche auf Basis der Schritte in diesem Practical die Performanz von Regression, Lasso, Ridge, Decision Tree und Random Forest für die `graduation_train` und `graduation_test` Datensätze aus dem letzten Practical. Lass die Modelle das Feature `Preis` auf Basis aller anderen Feature vorhersagen.
