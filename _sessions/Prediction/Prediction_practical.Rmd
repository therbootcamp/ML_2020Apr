...
### B - Lade den `graduation` Datensatz
...
9. Again,  We need to do a little bit of data cleaning. Specifically, we need to convert all character columns to factors: Do this by running the following code:
...
Your goal in this set of tasks is again to fit models predicting `Abschlussrate`, the percentage of attendees who graduate from each college.
...
15. Which one had the best fit? What was the fitting MAE of each model?
...
9. Again, we need to do a little bit of data cleaning. Convert all character columns to factor.
...
6. Using `train()`, fit a decision tree model called `price_rpart` predicting `price` using all features in `house_train`. Specifically,...
...
10. Using `train()`, fit a random forest model called `price_rf` predicting `price` using all features in `house_train`. Specifically,...
