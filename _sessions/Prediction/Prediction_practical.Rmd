...
In diesem Practical wirst die modellbasierte Vorhersagen generieren und die Modelle anhand der Qualität dieser messen. 
...
5. Definiere wie zuvor ein `ctrl_none` Objekt mit dem wir im Traning die Samplingmethode auf `none`. Erst in der nächsten Session beginnen wir dies zu ändern.
...
4. Wiederum, führe den Code unten aus um sicherzustellen, dass alle `character` Feature als Faktoren vorliegen. 
...
4. Mittels `predict()` generiere die gefitteten Werden von `abschluss_glm` und speichere sie als `glm_fit` ab. 
...
7. Explore your `abschluss_rpart` object by looking at `abschluss_rpart$finalModel` and plotting it with `plot(as.party(abschluss_rpart$finalModel))`, what do you find?

7. Exploriere `abschluss_rpart` indem du `abschluss_rpart$finalModel` printest und `plot(as.party(abschluss_rpart$finalModel))` den Decision Tree plottest. Wie beurteilst du die Outputs?
...
8. Mittels `predict()` generiere die gefitteten Werden von `abschluss_rpart` und speichere sie als `rpart_fit` ab.
...
10. Verwende `train()` um einen Random Forest namens `abschluss_rpart` zu fitten, welcher die `Abschlussrate` als Funktion aller anderen im Datensatz enthaltenen Features vorhersagt. D.h.,...
...
11. Mittels `predict()` generiere die gefitteten Werden von `abschluss_rf` und speichere sie als `rf_fit` ab.
...
12. Printe `glm_fit` und kriere ein Histogramm der Werte mittels `hist()`. Sind die Werte in einer plausiblen Region?
...
14. Verwende `posResample()` um die Fitting Performanz der drei Modelle zu vergleichen. Setze hierfür das `obs` Argument auf das eben erstellte `criterion_train` und das `pred` Argument auf die gefitteten Werte des jeweiligen Modells (z.B. `glm_fit`).
...
3. Verwende wiederum `posResample()` um nun die *Vorhersage* Performanz der drei Modelle zu vergleichen. Setze hierfür das `obs` Argument auf das eben erstellte `criterion_test` und das `pred` Argument auf die gefitteten Werte des jeweiligen Modells (z.B. `glm_pred`).
...
1. Fitte eine Regression, einen Decision Tree und einen Random Forest für `house_train.csv` wobei du `Preis` durch alle anderen Feature im Datensatz vorherzusagen versuchst.      
...
2. Exploriere die Objekte in dem zu `XX$finalModel` inspezierst und `summary()` anwendest. 

3. Verwende `predict()` um jeweils für die drei Modell die vorhergesagten Werte separat für den Trainingsdatensatz und für den Testdatensatz. Kreiere also drei Objekte mit Namen `XX_fit` und drei Objekte mit Namen `XX_pred`. 


4. Using `predict()`, save the fitted values of `preis_glm` object as `glm_fit`.
...
5. Speichere die Kriteriumswerte im Trainings- und Testdatensatz separat als `criterion_train` und `criterion_test` ab. 
...
6. Verwende `posResample()` um nun die *Fitting* und *Vorhersage* Performanz der drei Modelle zu vergleichen.
...
8. Welches Modell würdet ihr nun für den Einsatz in der Praxis zu empfehlen? Und welche Vorhersagegenauigkeit ist in der Realität zu erwarten?

### X - Challenges: Model Tuning Parameter

1. In allen Decision Trees wurde bisher der Komplexitätsparameter `cp` auf `0.01` gesetzt. Probiere nun aus was passiert, wenn du an ihm etwas drehst. Setze ihn z.B. auf `0.2` und schaue dir dann an wie Plot des Baums (`plot(as.party(XX_rpart$finalModel))`) sich verwendet. Werden die Bäume komplizierter? Wie verändert sich Fit- und Vorhersageperformanz?

2. In allen Random Forests wurde bisher der Paramater `mtry` auf `2` gesetzt. Probiere nun aus was passiert, wenn du an ihm etwas drehst. Setze ihn z.B. auf `5`  und beobachte wie sich sich Fit- und Vorhersageperformanz verändern und auch die Zeit, die für das Traning des Modells beansprucht wird?
...
# Schritt 1: Load and Clean, and Explore Training data ----------------------
...
glm_fit <- predict(hwy_glm, newdata = data_test)
rpart_fit <- predict(hwy_rpart, newdata = data_test)
rf_fit <- predict(hwy_rf, newdata = data_test)
...
Die `house_train` und `house_test` Datensätz enthalten Hauspreise und -Eigenschaften in der King County in den USA, welches die Stadt Seattle beinhaltet. Quelle der Daten ist [Kaggle](https://www.kaggle.com/harlfoxem/housesalesprediction).
