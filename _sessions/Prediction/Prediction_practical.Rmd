...
6. Using `train()`, fit a decision tree model called `grad_rpart`
...
8. Using `predict()` save the fitted values of `grad_rpart` object as `rpart_predfit`:
...
9. Print your `rpart_predfit` object, look at summary statistics with `summary(rpart_predfit)`, and create a histogram with `hist()` do they make sense?
...
10. Using `train()`, fit a random forest model called `grad_rf`
...
12. Print your `rf_fit` object, look at summary statistics with `summary(rf_fit)`, and create a histogram with `hist()` do they make sense?
...
14. Using `postResample()`, determine the fitting performance of each of your models separately. Make sure to set your `criterion_train` values to the `obs` argument, and your true model fits `XX_fit` to the `pred` argument:
...
15. Which one had the best fits? What was the fitting MAE of each model?
...
In this section, we will predict the prices of houses in King County Washington (home of Seattle, [which you can thank for this](https://qz.com/208457/a-cartographic-guide-to-starbucks-global-domination/)) using the `house_train` and `house_test` datasets.
...
9. We need to do a little bit of data cleaning before starting. Specifically, we need to convert all character columns to factor: Do this by running the following code:
...
6. Using `train()`, fit a decision tree model called `price_rpart` predicting `price` using all features in `house_train`.
...
7. Explore your `price_rpart` object by looking at `price_rpart$finalModel` and plotting it with `plot(as.party(price_rpart$finalModel))`, what do you find?
...
8. Using `predict()` save the fitted values of `price_rpart` object as `rpart_predfit`:
...
10. Using `train()`, fit a random forest model called `price_rf` predicting `price` using all features in `house_train`.
...
11. Using `predict()` save the fitted values of `price_rf` object as `rf_fit`:
...
|File  |Rows | Columns |
|:----|:-----|:------|
|[college_train.csv](https://raw.githubusercontent.com/therbootcamp/ML_2019May/master/1_Data/college_train.csv?token=AGKBX5SLEV3PLWUVQ4NCUB2427V36)| 500 | 18|
|[college_test.csv](https://raw.githubusercontent.com/therbootcamp/ML_2019May/master/1_Data/college_test.csv?token=AGKBX5UFLURQIE32GVYJR4S427WAY)| 277 | 18|
|[house_train.csv](https://raw.githubusercontent.com/therbootcamp/ML_2019May/master/1_Data/house_train.csv?token=AGKBX5THYI3PSBE5AO4KKBK427WD2)| 5000 | 21|
|[house_test.csv](https://raw.githubusercontent.com/therbootcamp/ML_2019May/master/1_Data/house_test.csv?token=AGKBX5WJAEXMFSV57RURV3S427WKM)| 1000 | 21|
